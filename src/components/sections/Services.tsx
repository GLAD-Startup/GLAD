...
export default function Services() {
  ...
  return (
    <section ref={sectionRef} id="services" className="relative w-full bg-[#0A0A0B] text-[#FBFBF9] select-none">
      {/* 1. Headline */}
      <div className="pt-[24px] md:pt-[30px] xl:pt-[36px] px-[20px] md:px-[28px] xl:px-[40px] overflow-hidden">
        <div ref={titleContentRef} className="will-change-transform">
          <h2
            className="t-display text-[#FBFBF9] whitespace-nowrap"
            style={{
          ...
          >
            Services
            <sup
              className="font-normal tracking-normal ml-1 text-[#A8A8AD]"
      ...
      <div className="mx-[20px] md:mx-[28px] xl:mx-[40px] mt-[48px] xl:mt-[72px]">
        <div
          ...
          className="xl:ml-[35%] xl:w-[65%] border-t border-white/[0.10]"
        >
          {servicesWithImages.map((service, idx) => {
            ...
            return (
              <Link
                ...
                className="group block py-[28px] md:py-[36px] xl:py-[42px] px-2 xl:px-4 border-b border-white/[0.10] transition-colors duration-300 hover:bg-[#141416]"
              >
                {/* Desktop 3-Column Grid (>=1200px): [80px 260px 1fr] */}
                <div className="hidden xl:grid grid-cols-[80px_260px_1fr] items-start gap-8">
                  <div className="t-label text-[#A8A8AD] pt-0.5 flex items-center">
                    <span>{service.index}</span>
                  </div>
                  <h3 className="t-title-lg text-[#FBFBF9] font-medium transition-transform duration-300 ease-out group-hover:translate-x-[6px]">
                    {service.title}
                  </h3>
                  <p className="t-body-sm text-[#A8A8AD] leading-relaxed max-w-[480px]">
                    {service.description}
                  </p>
                </div>
                ...
                <div className="grid xl:hidden grid-cols-[50px_1fr] md:grid-cols-[60px_200px_1fr] gap-3 md:gap-6 items-start">
                  <div className="t-label text-[#A8A8AD] flex items-center">
                    <span>{service.index}</span>
                  </div>
                  <div>
                    <h3 className="t-title-lg text-[#FBFBF9] font-medium">{service.title}</h3>
                    <p className="t-body-sm text-[#A8A8AD] mt-2 md:hidden">{service.description}</p>
                  </div>
                  <p className="t-body-sm text-[#A8A8AD] hidden md:block">{service.description}</p>
                </div>
              </Link>
            );
          })}
      ...
      <div
        ref={previewRef}
        className={clsx(
          'fixed top-0 left-0 -translate-x-1/2 -translate-y-1/2 w-[140px] h-[180px] xl:w-[155px] xl:h-[195px] rounded-[10px] overflow-hidden z-50 pointer-events-none shadow-2xl shadow-black/80 border border-white/[0.12] bg-[#141416] will-change-transform hidden min-[1024px]:block',
