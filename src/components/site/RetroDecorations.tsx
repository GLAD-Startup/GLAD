...
export function RetroStar({ className = "", color = "var(--brand-pink)", size = 48, rotation = 12 }: DecoProps) {
  return (
    <motion.div
    ...
    >
      <svg viewBox="0 0 100 100" fill={color} className="w-full h-full drop-shadow-[2px_2px_0px_rgba(0,0,0,1)] dark:drop-shadow-[2px_2px_0px_var(--color-foreground)]">
...
export function CurlyArrow({ className = "", color = "var(--brand-purple)", size = 64, rotation = 0 }: DecoProps) {
  return (
    <motion.div
      className={`absolute pointer-events-auto cursor-pointer ${className}`}
      style={{ width: size, height: size, rotate: rotation }}
    ...
    >
      <svg viewBox="0 0 100 100" fill="none" stroke={color} strokeWidth="6" strokeLinecap="round" className="w-full h-full">
...
export function SmileyBadge({ className = "", color = "var(--brand-2)", size = 56, rotation = -8 }: DecoProps) {
  return (
    <motion.div
    ...
    >
      ...
      >
        <svg viewBox="0 0 24 24" fill="none" stroke="#000" strokeWidth="2.5" strokeLinecap="round" className="w-2/3 h-2/3">
...
export function CuteSpeechBubble({ className = "", text = "Ship it!", color = "var(--background)", size = 120, rotation = 5 }: { className?: string; text: string; color?: string; size?: number; rotation?: number }) {
  return (
    <motion.div
    ...
    >
      ...
      >
        ...
        <div className="absolute bottom-[-9px] left-1/2 -translate-x-1/2 w-0 h-0 border-l-[8px] border-l-transparent border-r-[8px] border-r-transparent border-t-[8px]" style={{ borderTopColor: color }} />
      </div>
    </motion.div>
  );
}
